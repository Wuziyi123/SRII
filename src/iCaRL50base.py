...
class iCaRLmodel:
    ...
    def __init__(self, numclass, feature_extractor, batch_size, task_size, memory_size, epochs, learning_rate):
        ...
        self.model = network(40+numclass, feature_extractor)
        self.exemplar_set = []
        self.class_mean_set = []
    ...
    def beforeTrain(self):
        ...
        classes = [40 + self.numclass - self.task_size, 40 + self.numclass]
        # classes = [70, 80]
        self.train_loader, self.test_loader = self._get_train_and_test_dataloader(classes)
        if self.numclass > self.task_size:
            self.model.Incremental_learning(40 + self.numclass)
        self.model.train()
    ...
    def train(self, incremental, iter, up_model=None, branch=False):
        ...
        for epoch in range(self.epochs):
            ...
            for step, (indexs, images, target) in enumerate(self.train_loader):
                ...
                if epoch == 99 and step == end and incremental == 1 and iter == 1:
    ...
    def _compute_loss(self, indexs, imgs, target, branch=False):
        ...
        target = get_one_hot(target, 40 + self.numclass)
        output, target = output.to(device), target.to(device)
    ...
    def afterTrain(self, accuracy, incremental, branch=False):
        self.model.eval()
        m = int(self.memory_size / (40 + self.numclass))
        ...
        if not incremental:
            ...
            for i in range(40 + self.numclass - self.task_size, 40 + self.numclass):
                print('construct class %s examplar:' % (i), end='')  # 10-20
                images = self.train_dataset.get_image_class(i)
                self._construct_exemplar_set(images, m)
