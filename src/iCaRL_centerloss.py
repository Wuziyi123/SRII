...
class iCaRLmodel:
    ...
    def new_beforeTrain(self, i, up_model):
        ...
        # 5500 up   ---   5000 down  所以down需要从5000扩充到5500
    ...
    def updateBN(self, model):
        ...
        for name, m in model.named_modules():
            GBF = name.endswith("bn_g2_up")
            if isinstance(m, nn.BatchNorm2d) and not GBF:
    ...
    def train(self, incremental, iter, up_model=None, branch=False):
        ...
        for epoch in range(self.epochs):
            ...
            if not branch:  # 第一个增量过程只有上分支
                for step, (indexs, images, target) in enumerate(self.train_loader):  # 下分支的train_loader
                    ...
                    if incremental and not iter:  # 非稀疏、增量过程
                        ...
                        for name, m0 in model.named_modules():
                            ...
                            # exit(-1)
                            GBF = name.endswith("bn_g2_up")
                            if isinstance(m0, nn.BatchNorm2d) and not GBF:
                                bn_mask = torch.ones_like(end_mask) - end_mask
                                # 对于mask=1的部分为保留(冻结)的部分，此处的BN层参数应当清零(梯度先不管)
                    ...
                    # 混合精度训练上下文管理器，如果在CPU环境中不起任7何作用
                    with amp.autocast(enabled=enable_amp):
                        if incremental and iter == 1:  # 增量稀疏
                            pass
                        elif incremental and iter == 2:  # 增量精调
                            ...
                            if new.numel() and old.numel():
                                ...
                                idx_start = torch.floor(torch.tensor(sort_margin.shape[0] * 0.72)).long()  # 最好0.72
                    ...
                    if iter == 1:
                        self.updateBN(model)
                    elif incremental and not iter:  # 非稀疏、增量过程
                        # print(self.model)
                        # exit(-1)
                        cfg_mask = self.cfg_mask
                        layer_id_in_cfg = 0
                    ...
                    if epoch == 99 and step == end and incremental == 1 and iter == 0:  # 稀疏、增量过程
                ...
                for step, (indexs, images, target, up_images, up_targets) in enumerate(self.train_loader):  # 下分支的train_loader
                    images, target = images.to(device), target.to(device)
                    ...
                    # a = 0.5 * math.cos(0.015707963265 * epoch)  # cos函数参与度
                    # a = -1

                    # a = max(0.5 - epoch / 0.8 * 2
                    # T, 0)  # 一次函数参与度，Baseline
                    # a = 0.5 * sin((2π / 2T) * epoch)  # sin函数参与度
                    # a = 0.5 * cos((2π / 4T) * epoch)  # cos函数参与度
                    # a = max(0.5 - (0.5 / (T ^ 2)) * epoch ^ 2, 0)  # 二次函数参与度
                    ...
                    with amp.autocast(enabled=enable_amp):
                        ...
                        loss_up = loss1 + loss2 + 0.36 * margin_loss  # 0.27
                    ...
                    if epoch == 99 and step == end and incremental == 1 and iter == 1:  # 稀疏、增量过程
        ...
        for setp, (indexs, imgs, labels) in enumerate(testloader):
            ...
            if not branch:
                ...
                with torch.no_grad():
                    ...
                    _, outputs = self.model(imgs, material_from_down, 1)
    ...
    def afterTrain(self, accuracy, branch=False, down_model=None):
        ...
        torch.save(self.model, filename)  # 保存上分支模型
    ...
    def classify(self, test):
        ...
        class_mean_set = np.array(self.class_mean_set)  # 此处取回类范例中心,比较关键
        for target in test:
