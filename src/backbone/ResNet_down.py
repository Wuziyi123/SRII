...
__all__ = ['ResNet', 'branch_resnet18_cbam', 'branch_resnet34_cbam', 'branch_resnet50_cbam', 'resnet101_cbam',
           'resnet152_cbam']
...
class ResNet_down(nn.Module):
    ...
    def down_fusion(self, x):
        ...
        # l1 = layer1.clone().detach()
        # x1n = self.conv_gff1(l1)
        # g1 = self.conv_gff1_1(x1n)
        # g1 = torch.tanh(g1)

        # x2n = self.conv_gff2(layer2)
        # # g2 = self.conv_gff2_1(x2n)
        # g2 = torch.tanh(x2n)

        # x2n = layer2.clone().detach()
        # g2 = self.conv_gff2(x2n)
        # g2 = torch.tanh(g2)

        # g2 = self.conv_gff2(layer2)
        # g2 = torch.sigmoid(g2)

        # x3n = self.conv_gff3(layer3)
        # g3 = self.conv_gff3_1(x3n)
        # g3 = torch.sigmoid(g3)

        x3n = layer3.clone().detach()
        g3 = self.conv_gff3(x3n)
        g3 = torch.tanh(g3)
        # g3 = torch.add(torch.mul(g3, -50), 25)
        # g3 = torch.ones_like(layer3) / torch.add(torch.exp(g3), 1)
        ...
        return None, None, None, None, x3n, g3
    ...
    def forward(self, x, fusion_material_from_downBranch=None, once=False, getValue=False):
        if getValue:
            x = self.down_fusion(x)
        else:
...
def branch_resnet34_cbam(pretrained=False, **kwargs):
    """Constructs a ResNet-34 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_down(BasicBlock, [3, 4, 6, 3], **kwargs)
    if pretrained:
...
def branch_resnet50_cbam(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_down(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
...
def resnet101_cbam(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_down(Bottleneck, [3, 4, 23, 3], **kwargs)
    if pretrained:
...
def resnet152_cbam(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_down(Bottleneck, [3, 8, 36, 3], **kwargs)
    if pretrained:
